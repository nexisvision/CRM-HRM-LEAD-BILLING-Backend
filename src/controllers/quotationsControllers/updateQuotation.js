...
export default {
    ...
    handler: async (req, res) => {
        try {
            ...
            const existingQuotation = await Quotations.findOne({ where: { related_id: id, valid_till, currency, client, calculatedTax, items, discount, tax, total, id: { [Op.not]: id } } });
            if (existingQuotation) {
                return responseHandler.error(res, "Quotation already exists");
            }
            await quotations.update({ valid_till, currency, client, calculatedTax, items, discount, tax, total, updated_by: req.user?.username });
